...
{
    // DONE Cache
    // We need to define these things here
    // a valid bit
    int *valid;
    // a tag
    int *tag;
...
void iplc_sim_LRU_replace_on_miss(int index, int tag)
{
    ...
    /** 
        for loop will go through the given index's valid and tag
        to find the first zero then replace that with the tag
    **/
    for (i = 0; i < cache_assoc; i++) {
        if (cache[index].valid[i] == 0) {
            ...
            **/
            ...
            replaced_zero = 1;
            //we can stop looking after we replace the first zero
            break;
        }
    }
    //no zero was replaced so all indicies are filled so we have to 
    //shift all elements and fill the last index in the array with the new tag
    if (replaced_zero == 0) {   
        for (i = 0; i < cache_assoc-1; i++) {
            //the valid array doesnt need to be changed, all is still valid
...
{   
    ...
    //checks if the tag we are moving to the end is not at the end already
    if (i < cache_assoc - 1) {  //it is not
        //while loop checks that we are still in bounds
        //and if the next tag has not been filled in yet
        while ((i < cache_assoc-1) && (cache[index].tag[i+1] != 0)) {
            //replace the current tag with the next tag
            cache[index].tag[i] = cache[index].tag[i+1];
            i++;
        }
    }
    //whether the tag was at the end or not, i should indicate the index we move the tag to
    cache[index].tag[i] = moved_tag;
    cache[index].valid[i] = 1;
    cache[index].replace[i] = cache[index].replace[i] + 1;
}
...
int iplc_sim_trap_address(unsigned int address)
{
    ...
    //the tag is the address without the index and offset bits
    ...
    /** 
        need to search the cache for the desired value
        look at index and loop through tag and valid (associativity) times
        if the value is there add a hit and go to LRU update on hit
        if the value is not there add a miss and go to LRU replace on miss
    **/
    for (i = 0; i < cache_assoc; i++) {
        if ((cache[index].tag[i] == tag) && (cache[index].valid[i] == 1)) {
            //there was a hit
            cache_hit++;
            hit = 1;    //for the sake of the return and a check
            //now update LRU because of the hit
            //the parameter assoc_entry in the fuction is our i here
            //i is keeping our place in the arrays within the cache
            iplc_sim_LRU_update_on_hit(index, i);
            //break becaues there was a hit and we are done
            break;
        }
    }
    if (hit == 0) {
        //there was a miss
        cache_miss++;
        //update LRU because of miss
        //these parameters are just the index and the tag
        iplc_sim_LRU_replace_on_miss(index, tag);
    }

    //remember to add one to the cache_access
    cache_access++;
