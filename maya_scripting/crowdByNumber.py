...
def createUI( pWindowTitle, pApplyCallback ):
    ...
    cmds.rowColumnLayout( numberOfColumns=2, columnWidth=[ (1, 200), (2, 60) ], columnOffset=[ (1, "right", 5) ] )
    
    #using layout, add text, input fields, button, etc
    cmds.text( label="Number of crowd members:" )
    # numPeopleField lets user provide number of crowd members, defaults to 25
    numPeopleField = cmds.intField( value=25 )
...
# a simple message    
def applyCallback( pNumPeopleField, *pArgs ):
    print( "Apply button pressed" )
    
    #get the input by querying the intFields
    numPeople = cmds.intField( pNumPeopleField, query=True, value=True )
    # create the crowd
    createCrowd( numPeople )
